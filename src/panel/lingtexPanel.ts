      ...
      try {
        ...
        if (msg?.type === 'importImageFigure') {
          ...
          if (editor) {
            ...
            relForTex = path.relative(docDir, dest.fsPath);
          } else if (rootFsPath) {
            relForTex = path.relative(rootFsPath, dest.fsPath);
          }
          ...
          const tex = [
            ...
            '\n',
            '% NOTE: You may need to adjust \\includegraphics options (width/height/keepaspectratio) to fit on the page.',
            '% Quick reference: https://www.overleaf.com/learn/latex/Inserting_Images',
            '% If the figure is too large, the compiler might crash until you reduce the size or constraints.',
            '\n\\begin{figure}[htbp!]',
            '\\centering',
            `\\includegraphics[width=\\linewidth,height=0.85\\textheight,keepaspectratio]{${relForTex}}`,
            '\\caption{Caption.}',
            `\\label{${label}}`,
            '\\end{figure}',
            ''
      ...
      <body>
        ...
        <script>
          ...
          document.getElementById('btnImportImageFigure').addEventListener('click', () => {
            ...
            vscode.postMessage({ type: 'importImageFigure', outputDir: outDir });
          });
